...
import { getBusinessById, getReviewsByBusiness, getActiveDeals } from '../db/repo'
import { RatingStars } from '../components/RatingStars'
import { BookmarkButton } from '../components/BookmarkButton'
import { DealCard } from '../components/DealCard'
import { ReviewForm } from '../reviews/ReviewForm'
import { sanitizeWebsite, sanitizePhone, getDirectionsLink } from '../utils/links'
...
export function Business() {
  ...
  const loadBusiness = async () => {
    ...
    try {
      ...
      if (businessData) {
        ...
        const businessReviews = await getReviewsByBusiness(id)
  ...
  const directionsLink = getDirectionsLink(business.lat, business.lng, business.address)
  const distance = userLocation
    ? haversineDistance(userLocation.lat, userLocation.lng, business.lat, business.lng)
    : null
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="bg-white rounded-2xl shadow-2xl p-8 md:p-12 border border-gray-100">
        <div className="flex justify-between items-start mb-8">
          <div className="flex-1">
            <div className="flex items-center gap-4 mb-6">
              <h1 className="text-4xl md:text-5xl font-bold text-gray-900 bg-gradient-to-r from-primary-600 to-primary-800 bg-clip-text text-transparent">{business.name}</h1>
            ...
            {business.hours.map((hour, i) => (
              <div key={i} className="flex justify-between items-center py-2 border-b border-gray-200 last:border-0">
                ...
                <span className="text-gray-600 font-medium">
                  {hour.open === 'closed' ? <span className="text-red-500">Closed</span> : `${hour.open} - ${hour.close}`}
                </span>
              </div>
            ))}
          </div>
        ...
        {deals.length > 0 && (
          <div className="mb-8">
            <h2 className="text-2xl font-bold mb-6 text-gray-900">Special Deals</h2>
        ...
        <div className="mb-8">
          ...
          {showReviewForm && (
            <div className="mb-6 p-8 bg-gradient-to-br from-gray-50 to-white rounded-2xl border border-gray-200 shadow-lg">
              <ReviewForm businessId={business.id} onSubmit={handleReviewSubmit} />
            </div>
          )}
          ...
          <div className="space-y-6">
            {reviews.length === 0 ? (
              ...
              reviews.map((review) => (
                <div key={review.id} className="bg-white rounded-xl p-6 border border-gray-200 shadow-md hover:shadow-lg transition-shadow">
